// Header File: deauth.h
#ifndef DEAUTH_H
#define DEAUTH_H

#include <Arduino.h> // For basic types like uint16_t
...
// Declare the sniffer function defined in the .ino file so deauth.cpp can use it to set the callback
extern IRAM_ATTR void sniffer(void *buf, wifi_promiscuous_pkt_type_t type);

// Declare the filter defined in type.h (or where it resides)
extern const wifi_promiscuous_filter_t filt;

// Declare global variables from the .ino file so deauth.cpp can access them
